...
class Customer extends TightModel implements RelationManyToManyInterface
{
  ...
  public function immortal() : bool{ return $this->is_alias() === false;}

	// is the customer from pre-app epoch
	public function is_legacy(){return !empty($this->get('legacy'));}
	...
	public static function make_alias_of($customer, $name=null) : CustomerInterface
  {
	...
	public function first_contact_info()
	{
		...
		elseif(!$this->is_new())
		{
			$res = current(Note::filter(['customer' => $this, 'date_stop'=> $min_date], ['order_by'=>'occured_on ASC', 'limit'=>[0,1]]));
			if($res)
			{
